...
GlobalKey<_CommentListState> commentListGlobalKey = GlobalKey();
...
class _CommentListState extends State<CommentList> {
  ...
  int pageNum2 = 1;
  int pageSize2 = 10;
  late final List<BookListCommentVo> _commentList = [];
  ...
  void _findLikeList() async {
    ...
    List<UserVo> list = await Api().blLikeList(data);
  ...
  void _findList() async {
    ...
    List<BookListCommentVo> list = await Api().blCommentList(data);
    if (list.isNotEmpty && mounted) {
      setState(() {
  ...
  void _findChildren(String prentId, String blId, int index) async {
    if (id != prentId) {
      setState(() {
        id = prentId;
        pageNum2 = 1;
    ...
    List<BookListCommentVo> list = await Api().blCommentList(data);
    if (list.isNotEmpty && mounted) {
      BookListCommentVo vo = _commentList[index];
      vo.children ??= [];
      setState(() {
  ...
  void update(String? prentId, String id) async {
    BookListCommentVo? detail = await Api().bookListDetail(id);
    if (detail != null) {
      if (detail.prentId == '0') {
        setState(() {
        ...
        for (BookListCommentVo vo in _commentList) {
  ...
  Widget _list() {
    ...
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        if (_select == 0) {
          if (index + 1 == _commentList.length && haveNext1) {
            _findList();
          }
          return _getCommentList(context, index);
        } else {
          if (index + 1 == _likeList.length && haveNext2) {
            _findLikeList();
          }
  ...
  List<Widget> _getChildren(List<BookListCommentVo> list) {
    List<Widget> widgets = [];
    for (var vo in list) {
      widgets.add(
        Container(
  ...
  Widget _commentWidget(BookListCommentVo vo) {
    return GestureDetector(
  ...
  Widget _replyWidget(BookListCommentVo vo) {
  ...
  Widget _moreWidget(BookListCommentVo vo, int index) {
    String txt = '展开更多回复';
    ...
    return GestureDetector(
      onTap: () => _findChildren(vo.id!, vo.blId!, index),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.start,
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
