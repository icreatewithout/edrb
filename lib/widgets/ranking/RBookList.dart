...
class RBookList extends StatefulWidget {
  RBookList(this.val) : super(key: ValueKey(val));

  final String val;

  @override
  State<RBookList> createState() => _RBookList();
}

class _RBookList extends State<RBookList> {
  ...
  void _findList() async {
    ...
    List<BookVo> list = [];
    if (_val == '1001001') {
      list = await Api().homeGreat(data);
    } else if (_val == '2001002') {
      list = await Api().top200(data);
    } else if (_val == '3001003') {
      list = await Api().top50(data);
    } else if (_val == '4001004') {
      list = [];
    }
    if (list.isNotEmpty) {
      if (!mounted) {
